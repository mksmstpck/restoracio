...
//	@Failure		401		{object}	dto.Message
...
func (h *Handlers) login(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusUnauthorized, dto.Message{Message: err.Error()})
	...
	if ok := utils.CheckPasswordHash(creds.Password, admin.Password, admin.Salt); ok != true {
		c.JSON(http.StatusBadRequest, dto.Message{Message: "invalid password"})
